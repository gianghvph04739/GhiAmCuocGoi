...
@TargetApi(18)
public class FormatM4A extends com.audio.libary.encoders.MuxerMP4 {
    ...
    public FormatM4A(Context context, EncoderInfo info, FileDescriptor out) {
        Map<String, MediaCodecInfo> map = com.audio.libary.encoders.MuxerMP4.findEncoder(CONTENTTYPE_MP4);
        if (map.isEmpty())
            throw new RuntimeException("mp4 not supported");
        MediaFormat format = com.audio.libary.encoders.MuxerMP4.getDefault(CONTENTTYPE_MP4A, map);
